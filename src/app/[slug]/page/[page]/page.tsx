...
export async function generateStaticParams() {
  ...
  try {
    ...
    console.error('Error generating static params for [slug]/page/[page]:', err);
    const pages = ['1', '2', '3'];
    const params: Array<{ slug: string; page: string }> = [];
...
export async function generateMetadata({ params }: SlugPaginatedProps) {
  const resolvedParams = await params;
  const rawCat = resolvedParams?.slug || '';
  ...
  const url = `${baseUrl}/${categorySlug}/page/${pageNum}/`;
...
export default async function SlugCategoryPagePaginated({ params }: SlugPaginatedProps) {
  const resolvedParams = await params;
